...
/**
 * Импорт базы городов с сайта: http://download.geonames.org/export/zip/
 *
 * На входе в конструктор необходимо указать:
 * - временную папку, в которую необходимо скачать данные;
 * - название архива, в котором расположены города (например, RU.zip, cities15000.zip  и т.д.). Полный список архивов см. http://download.geonames.org/export/zip/
 * - консумер города.
 *
 * Пример использования:
 * ```php
 * $importer = new Zip('/tmp/', 'GB_full.csv.zip', function($zip) {
 *     fwrite(STDOUT, "Consume city Zip: ");
 *     print_r($zip);
 *     fwrite(STDOUT, "\n");
 * });
 * $importer->process();
...
class Zip {
     ...
     */
    public $baseUrl = 'http://download.geonames.org/export/zip/';
